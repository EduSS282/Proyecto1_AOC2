...
Port ( 		valid_I_ID : in  STD_LOGIC; --indica si es una instrucción válida			
			...
			undef: out STD_LOGIC; --indica que el código de operación no pertenence a una instrucción conocida. En este procesador se usa sólo para depurar
		   	...
		   	RTE	: out  STD_LOGIC; -- indica que es una instrucción RTE	   
				f_inc: out  STD_LOGIC -- Indicates that it is a fetch_inc instruction
			  -- Fin Nuevas señales
           );
end component;
-- NUEVO
...
Port (   	valid_I_ID : in  STD_LOGIC; --indica si es una instrucción válida
			...
			IR_op_code	: in  STD_LOGIC_VECTOR (5 downto 0); -- código de operación de la instrucción en IEEE
         	...
         	IO_MEM_ready: in std_logic; -- 1 cuando la memoria puede realizar la operación solicitada en el ciclo actual
			stall_MIPS: out  STD_LOGIC; -- Indica que todas las etapas deben parar
			Kill_IF		: out  STD_LOGIC; -- Indica que la instrucción en IF no debe ejecutarse (fallo en la predicción de salto tomado)
			stall_ID		: out  STD_LOGIC); -- Indica que las etapas ID y previas deben parar
end component;
...
--Señales solución
	signal stall_ID, stall_MIPS, Kill_IF, reset_ID, load_ID, load_EX, load_Mem, load_WB : std_logic;
-- Soporte Excepciones--
...
-- contadores
	...
	signal Ins, data_stalls, control_stalls, Mem_stalls, Exceptions: std_logic_vector(7 downto 0);
	signal inc_cycles, inc_I, inc_data_stalls, inc_control_stalls, inc_Mem_stalls, inc_Exceptions : std_logic;
...
-- Instrucciones jal y ret
	...
	signal jal_ID, ret_ID, jal_EX, jal_MEM, jal_WB : std_logic; --si necesitáis propagar las señales a otras etapas, definid las señales necesarias. Ejemplo: jal_EX, jal_MEM...	
...
begin
	...
	-- SOLUCION: se actualiza el PC si no se activan las señales de stall_ID o stall_MIPS
	...
	-- SOLUCION
	-- muxPC: este mux elige entre PC+4,la dirección de salto generada en ID, la dirección de la rutina de tratamiento de la excepción, o la dirección de retorno de la excepción
	-- cuidado con el orden, porque asigna prioridad si se cumplen dos condiciones			
	...
	PC_in <= 	x"00000008" 		when (Exception_accepted = '1') and (Data_abort = '1') else -- Si llega un data abort saltamos a la dirección 0x00000008
				...
				Exception_LR_output when RTE_ID = '1' else 	--@ retorno. Si es una RTE volvemos a la @ que teníamos almacenada en el Exception_LR		
				BusA				when RET_ID = '1' else 	--Se elige la dirección almacenada en el registro Rs que sale por el puerto A del BR
				Dirsalto_ID 		when salto_tomado = '1' else --@ Salto de las BEQ y JAL. Las RTE y RET también puede activar la señal de salto, pero por el orden del when se elegirán las opciones anterior
				PC4; -- PC+4
	...
	-- stall_ID detiene la etapa ID y ¡también debe parar la anterior! Eso último lo debéis hacer vosotros.
	-- Solución:
	...
	-- stall_ID: detiene la ejecución de las etpas ID e IF cuando hay riesgos
	...
	-- NUEVO: stall_MIPS: se utiliza para parar todo el procesador cuando la memoria no puede realizar la operación solicitada en el ciclo actual (es decir cuando IO_MEM_ready	-------------------------------------------------------------------------------------
	...
	-- EN el procesador de prácticas no se paraba nunca
	...
	------------------------------------------Etapa MEM-------------------------------------------------------------------
	--
	-- Nuevo: Módulo para gestionar las excepciones (completar)
	...
	-- En el interfaz también está la entrada IO_MEM_ready que nos avisa de si el subsistema de IO/MD va a realizar en este ciclo la orden del MIPS
...
-------------------------------------------------------------------------------------------------- 
	...
	-- Completar:
	inc_cycles <= '1';--Done
	inc_I <= valid_I_WB; --Sol
	inc_data_stalls <= stall_ID and not(stall_MIPS); --Sol
	inc_control_stalls <= kill_IF;--Sol
	inc_Exceptions <= Exception_accepted;--Sol
	inc_Mem_stalls <= stall_MIPS; --SOL
